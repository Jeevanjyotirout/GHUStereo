...
void visualize_and_record_disparity(
    ...
    for (int dx = -500; dx <= 500; dx = dx+130) {
        int x = center_x + dx;
        if (x >= 0 && x < disparity.cols) {

            float disp_val = disparity.at<float>(center_y, x);
            ...
            cv::circle(disp_color, cv::Point(x, center_y), 3, cv::Scalar(255, 0, 0), -1);
            cv::putText(disp_color, depth_text, cv::Point(x + 10, center_y - 10), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 0, 0), 2);
        }
    }
